...
func enter() -> void:
	...
	# 걷기 전용 다중 프레임 시트가 있는 종족은 그쪽을 쓰고, 없으면 기존 walk1/walk2 폴백.
	_use_animated_walk = pet.start_animated_walk()
	_anim_t = 0.0
	_frame = 0
	if _use_animated_walk:
		return
...
func exit() -> void:
	if _use_animated_walk:
		pet.stop_animated_walk()
		_use_animated_walk = false
	else:
		pet.walk_bob(false)
	if _face_mirrored:
		pet.mirror_face()
